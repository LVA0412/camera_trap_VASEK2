...
def button4():
    run1.config(text='Процесс запущен... ждите', state='disabled')
    win1.update()
    time.sleep(5)
    run1.config(text='Процесс выполнен', state='disabled')
    win1.update()
    time.sleep(2)
...
ttk.Button(win1, text="Посмотреть фото БЕЗ животными", command=button6).place(x=10, y=t+90)

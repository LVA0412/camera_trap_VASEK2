...
def processing_dataset(srcpath, targetpath, jsonpath):
    ...
    for json_image in jsondata['images'] :
        ...
        try:
            ...
            # уменьшаем размер
            gray_image = cv2.resize(gray_image,(0, 0), fx=RESIZE_SCALE_FACTOR, fy=RESIZE_SCALE_FACTOR)
            ...
            if av <=10 or av>=240:
                ...
                # if br == 0: # проверка color_channel_bad сильно замедляет и дает много ошибок, 
                #    далее это лучше проверить через calc_number_identical_bytes (дальше по коду)
